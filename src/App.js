...
const transliterateNameForLanguage = (value = '', language = 'English') => {
  // Proper names should stay unchanged unless AI explicitly returns a safe localized name.
  // The previous hardcoded name table only worked for a few demo names and caused fake localization.
...
const localizeClientText = (value = '', language = 'English') => {
  ...
  // No hardcoded demo phrase dictionaries. Real translation comes from the backend AI.
  // Returning the factual original lets safeClientLocalized decide whether to keep it or
  // fall back to generic localized text when the selected language requires translation.
  return text;
};
            ...
            <section className="public-section" key={section.id || section.name}>
              ...
              {section.items.map(item => (
                <article className="public-custom-item" key={item.id || `${item.heading}-${item.desc}`}>
                  {item.heading && (
                    item.link
                      ? <a href={item.link} target="_blank" rel="noreferrer" className="public-custom-heading-link"><strong>{item.heading}</strong></a>
...
${stripPortfolioMarkdownHeadingClient(finalLocalized.artistStatement || '')}`.trim();
        ...
        <div class="custom-items">
          ${s.items.map(it => {
            ...
            <div class="custom-item">
              ${it.heading ? (it.link
                ? `<a href="${it.link}" target="_blank" rel="noopener noreferrer" class="project-link custom-heading-link"><div class="custom-item-heading">${it.heading}</div></a>`
                : `<div class="custom-item-heading">${it.heading}</div>`) : ''}
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge { display: inline-block; background: linear-gradient(135deg, #8b5cf6 0%, #6d36dc 100%); border-radius: 20px; padding: 8px 20px; font-size: 0.8rem; color: white; font-weight: 500; }
  </style>
</head>
<body>
            ...
            <section className="hero-section" id="home">
              <div className="hero-container">
                <div className="hero-left">
                  <div className="hero-badge"><span className="badge-icon">✦</span>AI CREATIVE PARTNER · FACTLOCK VERIFIED</div>
              ...
              <div key={s.id} style={{marginTop:16}}>
                ...
                {s.items.map(it => (
                  <div key={it.id} className="result-custom-item">
                    {it.heading && (
                      it.link
                        ? <a href={it.link} target="_blank" rel="noreferrer" style={{display:'block',color:'#222',textDecoration:'none',marginBottom:6}}>
                            <strong style={{display:'block',color:'inherit'}}>{it.heading}</strong>
                          </a>
                        : <strong style={{display:'block',color:'#222'}}>{it.heading}</strong>
                    )}
                    {it.desc && <span style={{display:'block',color:'#555',fontSize:'0.9rem',lineHeight:1.55}}>{it.desc}</span>}
                    {it.media && (
                      <div style={{marginTop:8}}>
