...
const CV_SECTION_ALIASES = {
  ...
  certifications: ['CERTIFICATIONS', 'CERTIFICATES', 'COURSES', 'WORKSHOPS & CERTIFICATIONS', 'WORKSHOPS AND CERTIFICATIONS', 'TRAINING & CERTIFICATIONS'],
  awards: ['AWARDS', 'ACHIEVEMENTS', 'HONORS', 'HONOURS'],
  extracurricular: ['EXTRACURRICULAR ACTIVITIES', 'ACTIVITIES', 'VOLUNTEERING', 'VOLUNTEER EXPERIENCE', 'LEADERSHIP'],
...
function cvHeadingKey(line = '') {
  return CV_HEADING_TO_SECTION[normalizeCvHeading(line)] || '';
}
...
function cvSectionsFromLines(lines = []) {
  ...
  lines.forEach((line, index) => {
    const key = cvHeadingKey(line);
    if (key) headingIndexes.push({ index, key });
  });

  const sections = {};
  headingIndexes.forEach((entry, idx) => {
    const nextIndex = headingIndexes[idx + 1]?.index ?? lines.length;
    if (!sections[entry.key]) sections[entry.key] = [];
    sections[entry.key].push(...lines.slice(entry.index + 1, nextIndex));
  });
  return sections;
}
...
function sortEmbeddedCvLinksReadingOrder(items = []) {
  return [...items].sort((a, b) => {
    ...
    if (Math.abs((a.y || 0) - (b.y || 0)) > 1) return (a.y || 0) - (b.y || 0);
    return (a.x || 0) - (b.x || 0);
  });
}
...
function parseCvSkills(skillLines = []) {
  const entries = bulletItemsFromLines(skillLines);
  const source = entries.length ? entries : skillLines.map(cleanCvLine).filter(Boolean);
  const skills = [];
  ...
  source.forEach(entry => {
    ...
    rest.split(/,|\||;/g).forEach(part => {
      const item = cleanCvLine(part).replace(/[.]$/, '');
      if (item.length > 1 && item.length <= 60 && !/^(and|with|basic)$/i.test(item)) skills.push(item);
    });
  });
...
function proofLinksFromEmbedded(embeddedLinks = []) {
  return sortEmbeddedCvLinksReadingOrder(embeddedLinks)
    .map(item => normalizeCvUrl(item.url))
    .filter(Boolean)
...
function parseCvExperience(experienceLines = []) {
  const raw = experienceLines.map(line => stripCvBullet(line)).map(cleanCvLine).filter(Boolean);
  if (!raw.length) return [];

  if (raw.length >= 2 && !/^(Designed|Collaborated|Built|Developed|Implemented)\b/i.test(raw[0])) {
    return [{ heading: raw[0], desc: cleanCvLine(raw.slice(1).join(' ')), link: null }];
  }

  return bulletItemsFromLines(experienceLines).map(item => {
...
function parseCvCustomSections(sections = {}, embeddedLinks = []) {
  ...
  const proofLinks = proofLinksFromEmbedded(embeddedLinks);
  let proofIndex = 0;

  if (Array.isArray(sections.certifications) && sections.certifications.length) {
    const items = bulletItemsFromLines(sections.certifications).map(item => {
      const { heading, desc } = splitHeadingDesc(item);
      const link = proofIndex < proofLinks.length ? proofLinks[proofIndex] : null;
      proofIndex += 1;
      return { heading, desc, link };
    });
  ...
  if (Array.isArray(sections.awards) && sections.awards.length) {
    const items = bulletItemsFromLines(sections.awards).map(item => {
      const { heading, desc } = splitHeadingDesc(item);
      return { heading, desc, link: null };
    });
  ...
  if (Array.isArray(sections.extracurricular) && sections.extracurricular.length) {
    const items = bulletItemsFromLines(sections.extracurricular).map(item => {
      const { heading, desc } = splitHeadingDesc(item);
      return { heading, desc, link: null };
    });
