...
const CV_SECTION_ALIASES = {
  ...
  extracurricular: ['EXTRACURRICULAR ACTIVITIES', 'EXTRACURRICULAR', 'ACTIVITIES', 'VOLUNTEERING', 'VOLUNTEER EXPERIENCE', 'VOLUNTEER WORK', 'LEADERSHIP', 'LEADERSHIP EXPERIENCE', 'CO CURRICULAR ACTIVITIES', 'EXTRA CURRICULAR ACTIVITIES', 'EXTRA CURRICULAR', 'COMMUNITY INVOLVEMENT'],
...
function joinWrappedCvHeadings(lines = []) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    // Join a heading wrapped across two lines (e.g. "PROJECT" / "EXPERIENCE").
    // Guard on `current` only: if the first line is ALREADY a heading on its own, these are
    // two separate sections and must not merge. `next` may be a heading by itself
    // (e.g. "EXPERIENCE") — the two-word form ("PROJECT EXPERIENCE") is more specific and
    // must win, otherwise project entries get misfiled under Experience.
    if (next && !isCvSectionHeading(current)) {
      const combined = `${current} ${next}`.trim();
      if (combined.length <= 40 && isCvSectionHeading(combined)) {
        out.push(combined);
